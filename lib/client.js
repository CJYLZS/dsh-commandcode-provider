window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ".cps-page{display:flex;flex-direction:column;gap:12px;max-width:760px}",
      ".cps-intro{margin:0;color:var(--dsw-alias-label-secondary);font-size:13px;line-height:20px}",
      ".cps-card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:8px;min-width:0;overflow:hidden}",
      ".cps-head{align-items:center;gap:8px;padding:10px 14px;display:flex}",
    ...
    const I18N = {
      zh: {
        ...
        sectionIntro: "把所选订阅档位的模型同步进 dsh 的模型供应商；同一个账号还可用于 web 搜索与用量查看。",
        unsaved: "未保存",
        plan: "订阅类型",
        ...
        includeEfforts: "写模型思考档位映射（建议开启）",
        includeEffortsHint: "给目录里标了 reasoning 的模型写入 low…max 挡位——这正是 CommandCode 接口接受的字面量。关掉后 dsh 会把模型当作不支持思考：选择器里没有挡位，请求也不带思考参数。",
        autoSync: "自动同步",
      ...
      en: {
        ...
        sectionIntro: "Sync the selected subscription's models into dsh's model providers; the same account also serves web search and the usage dashboard.",
        unsaved: "unsaved",
        plan: "Subscription",
        ...
        includeEfforts: "Write model reasoning effort maps (recommended)",
        includeEffortsHint: "Models the catalog marks as reasoning get the low…max levels — the literals the Command Code endpoint accepts. Turning this off makes dsh treat every model as non-reasoning: no levels in the picker and no reasoning parameter on the wire.",
        autoSync: "Auto sync",
    ...
    function CommandcodePlanSyncCard() {
      ...
      const [includeEfforts, setIncludeEfforts] = react.useState(true);
      ...
      const load = react.useCallback(async () => {
        try {
          ...
          if (view) {
            ...
            // Only an explicit false disables it: an absent value means the
            // schema default, which writes the maps.
            setIncludeEfforts(v.includeReasoningEfforts !== false);
      ...
      // Auto-fetch usage once per mount; later refreshes are explicit.
      react.useEffect(() => {
        if (usage.status === "idle") doRefreshUsage();
      }, []);
      ...
      if (state.status === "loading") {
        return react.createElement("div", { className: "cps-card" },
      ...
      return react.createElement("div", { className: "cps-card" },
        react.createElement("div", { className: "cps-head" },
    ...
    // The plugin manager's row page key: `<bundle package name>#<row id>`. The
    // row is this bundle's own patch row, whose Configure control opens the
    // page this card renders.
    const ROW_KEY = "dsh-commandcode-provider#dsh-commandcode-provider";

    // The card carries its own zh/en table, and the page's summary line is a
    // fallback for a row whose package declares no description.
    const uiLang = () => (String(globalThis.navigator?.language ?? "").toLowerCase().startsWith("zh") ? "zh" : "en");

    // The plugin's own page in the Settings panel: the plan-sync form, the
    // usage dashboard, and the web-search switch.
    function CommandcodeSection() {
      const t = tt(uiLang());
      return react.createElement("div", { className: "cps-page" },
        react.createElement("p", { className: "cps-intro" }, t.sectionIntro),
        react.createElement(CommandcodePlanSyncCard, null));
    }

    exports.inject = ["slots"];
    exports.apply = function apply(ctx) {
      // A section of its own in the Settings panel, beside the shipped pages.
      // The nav label carries the product name only: the page's own heading
      // and description say what it does.
      ...
      // The same page from the Plugins page: the bundle row's Configure control.
      ctx.effect(() => ctx.slots.inject("plugins.row.config", () => ctx.slots.register({
        name: "plugins.row.config",
        key: ROW_KEY,
      }, ({ view }) => {
        if (view === "summary") return tt(uiLang()).desc;
        return react.createElement(CommandcodePlanSyncCard, null);
      })));
    };
