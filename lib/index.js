...
export const Config = z.object({
  ...
  usageBaseURL: z.string().default(DEFAULT_ALPHA_BASE_URL),
});
...
export function apply(ctx, config) {
  ...
  const pluginConfigRef = () => ({
    ...
    usageBaseURL: current().usageBaseURL?.trim() || DEFAULT_ALPHA_BASE_URL,
  });

  // Re-applied on section attach/change (both settings API generations call
  // onChange at attach and on every committed change), so the search selection
  // always converges on the latest webSearch value once ctx.web exists.
  let searchController = null;
  const applySearchSelection = () => {
    if (!searchController) return;
    try { searchController(current().webSearch === true); } catch { /* no hackable selection field */ }
  };

  installSettingsCompat(ctx, PLAN_NS, Config, config ?? {}, {
    setSource: (src) => { current = src; },
    onChange: applySearchSelection,
  });
  ...
  ctx.inject(["web"], (wctx) => {
    const provider = makeSearchProvider({
      resolveKey: () => resolveApiKey(wctx, current().targetApiKeyEnv ?? DEFAULT_API_KEY_ENV),
      apiBase: () => pluginConfigRef().usageBaseURL,
      keyEnv: () => current().targetApiKeyEnv ?? DEFAULT_API_KEY_ENV,
    });
    const disposeProvider = wctx.web.registerSearchProvider(provider);
    wctx.effect(() => () => disposeProvider(), "dsh-commandcode-plan-autosync: search provider");
    ...
    }, "dsh-commandcode-plan-autosync: search selection");
