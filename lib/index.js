...
const PLAN_NS = settingsNamespace("dsh-commandcode-plan-autosync");
...
function buildEntries({ apiList, catalog, plan, extraIds }) {
  ...
  for (const api of apiList) {
    ...
    // reasoning:false -> block reasoning_effort for this model; true/unknown ->
    // omit so the provider-level compat decides (no official effort lists).
    if (cat && cat.reasoning === false) entry.reasoningEfforts = false;
...
function buildRoutePatch({ key, route, entries }, pluginConfig) {
  ...
  if (route === "openai") {
    patch.compat = pluginConfig.targetCompat ?? { thinkingFormat: "openai", supportsReasoningEffort: true };
  }
  return { key, patch };
}
...
async function upsertProvider({ settings, key, patch }) {
  ...
  if (!existing) {
    ...
    if (!existing.compat && patch.compat) ops.push({ op: "set", path: ["providers", key, "compat"], value: patch.compat });
  ...
  const handlers = {
    ...
    async sync(body) {
      ...
      try {
        ...
        const byRoute = buildEntries({ apiList, catalog, plan, extraIds: cfg.extraIds ?? [] });
        if (body?.dryRun) {
          return {
            ok: true,
            value: {
    ...
    async mutate(body) {
      ...
      try {
        await settings.mutate(settingsNamespace(body.ns), body.ops, rev);
      } catch (e) {
...
export const Config = z.object({
  ...
  plan: z.enum(["goat", "pro", "max"]).default("goat"),
...
export function apply(ctx, config) {
  ...
  installSettingsSection(ctx, PLAN_NS, Config, config ?? {}, {
  ...
  ctx.effect(() => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        const byRoute = buildEntries({ apiList, catalog, plan, extraIds: cfg2.extraIds ?? [] });
