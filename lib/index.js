...
function buildEntries({ apiList, catalog, plan, extraIds, includeReasoningEfforts = true }) {
  ...
  for (const api of apiList) {
    ...
    if (cat && cat.reasoning === false) {
      ...
      // The catalog says the model reasons but lists no levels, so the map is
      // the identity over the levels the endpoint itself accepts: a request
      // carrying any other value is refused with HTTP 400 naming this set.
      // dsh reads the map as the model's capability — without it the model is
      // non-reasoning, offering no levels and sending no reasoning parameter.
  ...
  const handlers = {
    ...
    async sync(body) {
      ...
      try {
        ...
        const byRoute = buildEntries({ apiList, catalog, plan, extraIds: cfg.extraIds ?? [], includeReasoningEfforts: cfg.includeReasoningEfforts ?? true });
        if (body?.dryRun) {
          return {
            ok: true,
            value: {
...
export const Config = z.object({
  ...
  includeReasoningEfforts: z.boolean().default(true).volatile(),
...
export function apply(ctx, config) {
  // Every field an operation reads must appear here: this snapshot is what
  // sync, auto-sync, and the bridge consume, so a field it omits takes the
  // fallback below no matter what the settings card saved.
  const pluginConfigRef = () => ({
    ...
    includeReasoningEfforts: config.includeReasoningEfforts.get() ?? true,
    autoSync: config.autoSync.get() ?? false,
    autoSyncIntervalMs: config.autoSyncIntervalMs.get() ?? 6 * 60 * 60 * 1000,
  ...
  const armAutoSync = () => {
    ...
    const tick = async () => {
      ...
      try {
        ...
        const byRoute = buildEntries({ apiList, catalog, plan, extraIds: cfg2.extraIds ?? [], includeReasoningEfforts: cfg2.includeReasoningEfforts ?? true });
